-- =============================================================================
-- Mestre 360 — Produção — v1.8.4
-- Correção definitiva do saldo bancário no estorno (valor somava 2x)
-- Idempotente. Rodar no SQL Editor do Supabase de PRODUÇÃO.
-- =============================================================================

-- 1) Valor assinado: estorno NÃO zera o lançamento original
--    (quem corrige o saldo é o contra-lançamento gerado pelo estorno)
...
-- 2) Função única de saldo: aplica somente em INSERT e DELETE
...
-- 2b) Função antiga neutralizada (caso ainda exista em algum trigger)
...
-- 3) Remove QUALQUER trigger de saldo duplicado em lancamentos e recria um único
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
-- 4) Recalcula os saldos atuais (saldo inicial + todas as movimentações)
