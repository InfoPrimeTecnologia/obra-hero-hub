-- =============================================================================
-- Mestre 360 — Produção — v1.8.0
-- Fatura de cartão rateada por obra + correção da troca de foto da obra
...
BEGIN
  ...
  EXCEPTION WHEN unique_violation OR duplicate_table THEN
    RAISE NOTICE 'Indice fatura+obra nao criado (registros duplicados existentes)';
  END;
END $$;

-- 4) Permite substituir a foto da obra (upsert no storage exige policy de UPDATE)
...
SELECT '1.8.0',
       'Fatura de cartão rateada por obra',
       '[
         "Dentro da obra, a fatura do cartão mostra apenas a parte daquela obra (soma das compras da obra)",
         "Na empresa, a fatura do cartão continua exibindo o total, com o detalhamento por obra",
         "Pagar dentro da obra quita somente a parte da obra; a fatura só fica paga quando todas as obras quitarem",
         "Contas a pagar da obra não duplicam mais a fatura do cartão",
         "Correção: troca da foto da obra estava sendo bloqueada"
       ]'::jsonb,
       now()
WHERE NOT EXISTS (SELECT 1 FROM public.app_releases WHERE version = '1.8.0');
